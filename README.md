...
- 测试：命令解析、JSON 输出、助手版本/运行环境/窗口 JSON 解析、工作区聚焦状态文案、诊断日志写入/导出、脱敏诊断包生成和导出目录命名、话术种子/搜索/常用短语、话术更新/删除、JSON/CSV 导入、账号持久化/备注/删除/排序/离线同步、隐私锁状态持久化、开源组件声明、运行包清单、安装清单、运行包文件校验、助手 SHA-256 校验、设置摘要、运行环境检查、试用/本地激活授权状态、云端激活响应解析/持久化、云端激活传输编排、授权功能限制和更新清单状态。
