...
- 本地账号管理：账号记录保存到 `accounts.json`，支持备注编辑、删除本地记录，启动/刷新微信窗口后更新账号状态，选中已检测窗口时通过助手组件请求聚焦。
...
- 测试：命令解析、JSON 输出、助手窗口 JSON 解析、话术种子/搜索、话术更新/删除、JSON/CSV 导入、账号持久化/备注/删除、开源组件声明、试用/本地激活授权状态。
