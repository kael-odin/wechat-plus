...
| `WeChatPlus.Install.exe` | Installer | Independent install entry that copies manifest-listed runtime files and creates a Start Menu `.lnk`, with an auditable fallback file when Windows Shell COM is unavailable. |
