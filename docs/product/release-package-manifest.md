...
`InstallerManifest` describes the default installer metadata without mutating the local machine: product name, publisher, default install directory under Program Files, Start Menu shortcut, uninstall command, uninstall registry key, runtime files to remove, shortcut cleanup, user-data retention policy, open-helper source URL, and the runtime package files. A future MSI/EXE installer should consume this metadata and then implement actual shortcut creation, registry uninstall registration, elevation, rollback, and optional user-data cleanup.
